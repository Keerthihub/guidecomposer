...
Mullion is an extension for Adobe Illustrator and Adobe InDesign published by
[PUBLISHER_NAME] ("we").

## What Mullion collects

Nothing. Mullion does not collect, transmit, sell, or share personal data or
usage data.

- **No network access.** Mullion makes no network requests of any kind. It
...
- **No accounts.** Mullion never asks for a sign-in, an email address, or a
...
Everything Mullion saves stays on your own computer. None of it is readable by
us.
...
- **Your documents** gain a layer named "Mullion grids" containing the grids you
  create. Grids carry small tags that identify them as Mullion grids, so Mullion
...
- **Preset files** are written only when you choose Export presets, to the
  ...
  Mullion reads no other file on your computer, and writes nowhere else.

## Purchases

If you bought Mullion through a store (for example the Adobe Creative Cloud
...
Mullion is a professional design tool. It is not directed at children and
...
If a future version changes what Mullion collects or stores, we will update this
