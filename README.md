...
| InDesign host | Tests against a strict fake InDesign DOM, including a dry run of the real QA script (`tests/indesign-qa.test.js`) | **`npm run qa:indesign` — 32 checks, 0 failures, in InDesign 2026 (21.6.0) on macOS 26.4.1**, in documents it creates and closes: text selections, localised stroke names, facing pages, the baseline reference point, page insertion, master spreads, guides, rollback, a 200-page document, and one named undo step. First run on 2026-09-23; it found two real bugs the simulation could not — see CHANGELOG. InDesign is still **not declared in the manifest**: shipping it is a separate decision |
| Panel | `npm run test:ui` — headless Chrome smoke test of every control and all three modes, in all four brightness themes, and down to the smallest panel the manifest allows. CI runs it on Ubuntu, macOS **and Windows** runners. **And `npm run qa:panel`** — the real panel driven inside a running Illustrator over CEP's debugging port, in a document it creates and closes: every module loads in CEP, every grid type generates, overlays, gallery tiles, and the preset backup file | Last run: 11 checks, 0 failures, Illustrator 30.8.1, macOS 26.4.1, panel 0.1.1. `qa:panel` needs the panel open, which no script can do for you |
| Windows | Unit tests, build and panel smoke test all run on a Windows runner in CI | The author reports the panel working on Windows. **No Windows build, Illustrator version, or install method was recorded** — in particular whether it was the signed `.zxp` or a development copy, which prove different things — so it cannot be re-verified or cited. Treat Windows as untested until a run is recorded here. `docs/WINDOWS-QA.md` is the sheet to fill in |
