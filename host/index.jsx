/*
 * GuideComposer host entry point (ExtendScript, ES3 only).
 *
...
(function (M) {
    ...
    function endpoint(fn) {
        return function (encoded) {
            if (!M.ready) {
                return '{"ok":false,"error":{"code":"NOT_READY","message":"GuideComposer is still starting. Try again in a moment.","fields":[]}}';
            }
            try {
    ...
    M.boot = function (encodedRoot) {
        ...
        try {
            ...
            for (var i = 0; i < DEPENDENCIES.length; i++) {
                ...
                if (!file.exists) {
                    return '{"ok":false,"error":{"code":"MISSING_FILE","message":' +
                        quote("GuideComposer is missing " + relative + ". Reinstall the extension.") + ',"fields":[]}}';
                }
                $.evalFile(file);
            }
            M.root = root;
            ...
            return '{"ok":false,"error":{"code":"BOOT_FAILED","message":' +
                quote("GuideComposer could not start: " + (err && err.message ? err.message : err)) + ',"fields":[]}}';
        }
    };
    ...
    M.api = {
        ...
        // replaces the GuideComposer grids already in those areas; mode "add" keeps
        ...
        // Removes GuideComposer grids (preview and generated). Artboard targets clear
